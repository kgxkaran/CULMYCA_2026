...
export default function RegistrationModal({ event, onClose, onPaymentRequired }) {
  const { user } = useAuthStore()
  const queryClient = useQueryClient()

  const isTeam = event.teamSizeMax > 1
  const minTeammates = event.teamSizeMin - 1  // excludes self
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: ({ data }) => {
      if (data.requiresPayment) {
        // Hand off to Razorpay flow
        onClose()
        onPaymentRequired(data.registration)
      } else {
        setRegistration(data.registration)
        setStep(STEPS.SUCCESS)
        // Invalidate events list so slot count updates
  ...
  const handleContinue = () => {
    if (validate()) setStep(STEPS.CONFIRM)
  }

  const handleSubmit = () => {
    mutate({ eventId: event._id, teamName, members })
  }
      ...
      >
        ...
        <div className="flex items-start justify-between px-6 pt-6 pb-4 border-b border-white/10 shrink-0">
            ...
            <div className="flex items-center gap-2 mb-1">
              ...
              {isTeam ? (
                <span className="flex items-center gap-1 text-xs text-dark-100">
                  <Users size={12} /> Team ({event.teamSizeMin}–{event.teamSizeMax})
                </span>
              ) : (
                <span className="flex items-center gap-1 text-xs text-dark-100">
                  <User size={12} /> Solo
          ...
          {step !== STEPS.SUCCESS && (
            <button
              onClick={onClose}
              className="text-dark-100 hover:text-white transition-colors ml-4 shrink-0"
            >
              <X size={20} />
            </button>
          )}
          ...
          <AnimatePresence mode="wait">
            ...
            {step === STEPS.DETAILS && (
              <motion.div
                ...
                className="space-y-5"
              >
                {/* Registering user info (read-only) */}
                <div className="rounded-lg bg-white/5 border border-white/10 p-4">
                  <p className="text-xs text-dark-100 mb-1 font-medium uppercase tracking-wide">
                    Registering as
                  </p>
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <p className="text-sm font-medium text-dark-100">
                          Teammates ({members.length}/{maxTeammates})
                        </p>
                        ...
                        {members.map((member, i) => (
                          ...
                          >
                            <div className="flex items-center justify-between mb-1">
                              <span className="text-xs font-medium text-dark-100 uppercase tracking-wide">
                                Member {i + 1}
                              </span>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <div>
                                ...
                                {errors[`member_${i}_name`] && (
                                  <p className="mt-0.5 text-xs text-red-400">{errors[`member_${i}_name`]}</p>
                                )}
                              </div>
                              <div>
                                <input
                                ...
                                {errors[`member_${i}_email`] && (
                                  <p className="mt-0.5 text-xs text-red-400">{errors[`member_${i}_email`]}</p>
                                )}
                              </div>
                              <div>
                                <input
                                ...
                                {errors[`member_${i}_phone`] && (
                                  <p className="mt-0.5 text-xs text-red-400">{errors[`member_${i}_phone`]}</p>
                                )}
                              </div>
                              <div>
                                <input
                                  ...
                                  onChange={(e) => updateMember(i, 'collegeName', e.target.value)}
                                  placeholder="College"
                                  className="input w-full text-sm"
                                />
                                {errors[`member_${i}_collegeName`] && (
                                  <p className="mt-0.5 text-xs text-red-400">{errors[`member_${i}_collegeName`]}</p>
                                )}
                              </div>
                            </div>
                          </div>
                ...
                {!isTeam && (
                  <p className="text-dark-100 text-sm">
                    This is a solo event. Just confirm your details above and proceed.
                  </p>
                )}
              </motion.div>
            )}
              ...
              >
                <div className="rounded-lg bg-white/5 border border-white/10 divide-y divide-white/10">
                  ...
                  <div className="px-4 py-3 grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                    ...
                    <div>
                      ...
                      <p className={event.isPaid ? 'text-yellow-400 font-medium' : 'text-green-400 font-medium'}>
                  ...
                  {isTeam && (
                    <div className="px-4 py-3">
                      ...
                      <p className="text-dark-100 text-xs mt-0.5">
                        {user.name} (you) + {members.length} member{members.length !== 1 ? 's' : ''}
                      </p>
                    </div>
                  )}
                </div>
                ...
                {event.isPaid && (
                  <div className="rounded-lg bg-yellow-500/10 border border-yellow-500/20 px-4 py-3 text-sm text-yellow-300">
                    You'll be redirected to Razorpay to complete payment of{' '}
                    <span className="font-bold">₹{event.registrationFee}</span>. Your spot is reserved for 10 minutes.
                  </div>
                )}
            ...
            {step === STEPS.SUCCESS && registration && (
              ...
              >
                ...
                <p className="font-mono text-primary-400 font-bold text-lg mb-6">
                  {registration.ticketId}
                </p>

                <p className="text-xs text-dark-100 mb-4">
                  View your ticket anytime in the Student Dashboard.
                </p>
